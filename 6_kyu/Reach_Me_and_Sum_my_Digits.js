...
function sumDigNthTerm(initval, patternl, nthterm) {
    var patternSum = 0;
    for(i=0; i<patternl.length; i++) {
      patternSum += patternl[i];
    }
    var patternRep = Math.floor((nthterm-1)/(patternl.length));
    var remainder = (nthterm-1)%(patternl.length);
    var remainderSum = 0;
    for(i=0; i<remainder; i++) {
      remainderSum += patternl[i];
    }
    var result = initval + patternSum*patternRep + remainderSum;
    var digitSum = 0;
    result = result.toString();
    for(i=0; i<result.length; i++) {
      digitSum += parseInt(result[i]);
    }
    return digitSum;
}